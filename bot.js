...
const prefix   = "#";
const category = "541921788528033792";
const devs     = ["349095859859881984"];
let mtickets   = true;
let tchannels  = [];
let current    = 0;

hero.login('NTQyNTY5ODk1MDcwNTMxNTk0.Dzv7Jw.YWeWa3mejK4h0ZLpJsRszGY6AE8');

hero.on('ready',async () => console.log(`   - " ${hero.user.username} " , Tickety is ready to work.`));
hero.on('message',async message => {
    const emojis   = { yes: `${hero.guilds.find(r => r.id === '541315161835962369').emojis.find(e => e.name === 'true')}`, wrong: `${hero.guilds.find(r => r.id === '541315161835962369').emojis.find(e => e.name === 'false')}` };
